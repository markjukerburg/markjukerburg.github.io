  ...
  createPredictBtn(){
    ...
    predButton.addEventListener('mousedown', () => {
      ...
      if(Math.max(...exampleCount) > 0){
        ...
        if(exampleCount[0] == 0){
          alert(
            `You haven't added examples for the wake word ADARSH`
            )
          return
        }
...
class TextToSpeech{
  ...
  speak(word){
    ...
    if(word != 'adarsh' && this.currentPredictedWords.length == 0){
      console.log("first word should be adarsh")
      console.log(word)
      return
    }
